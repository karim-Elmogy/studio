...
@section('title', __('admin.projects_management'))

@section('toolbar')
    <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
        <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">
            {{ __('admin.projects_management') }}
        </h1>
        <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
            ...
            <li class="breadcrumb-item text-muted">{{ __('admin.projects') }}</li>
        </ul>
    </div>
    ...
    <div class="card shadow-sm">
        ...
        <div class="card-body pt-0">
            <div class="table-responsive">
                <table class="table align-middle table-row-dashed fs-6 gy-5" id="projectsTable">
                    <thead>
                        <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">

                            <th class="min-w-100px">{{ __('admin.image') }}</th>
                            <th class="min-w-200px">{{ __('admin.project') }}</th>
                            <th class="min-w-125px">{{ __('admin.category') }}</th>
                            <th class="min-w-100px">{{ __('admin.type') }}</th>
                            ...
                            <tr>
                                    ...
                                    @else
                                        <div class="symbol symbol-50px me-3">
                                            <div class="symbol-label bg-light-primary">
                                                <i class="fa-solid fa-folder fs-2x text-primary"></i>
                                            </div>
                                        </div>
                                    @endif
                                </td>
                                    ...
                                    <div class="d-flex flex-column">
                                        <a href="{{ route('admin.projects.show', $project) }}"
                                           class="text-gray-800 text-hover-primary mb-1 fw-bold">
                                            {{ Str::limit($project->title[app()->getLocale()] ?? $project->title['en'] ?? 'N/A', 50) }}
                                        </a>
                                        @if($project->description && isset($project->description[app()->getLocale()]))
                                            <span class="text-muted fs-7">
                                                {{ Str::limit($project->description[app()->getLocale()], 60) }}
                                            </span>
                                        @endif
                                    </div>
                                </td>
                                <td>
                                    <span class="badge badge-light-info">
                                        {{ $project->category[app()->getLocale()] ?? $project->category['en'] ?? 'N/A' }}
                                    </span>
                                </td>
                                <td>
                                    <span class="badge badge-{{ $project->type === 'web' ? 'light-info' : 'light-success' }}">
                                        <i class="fa-solid fa-{{ $project->type === 'web' ? 'globe' : 'mobile-screen-button' }} me-1"></i>
                                ...
                                <td class="">
                                    ...
                                    <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4"
                                        ...
                                        <div class="menu-item px-3">
                                            <a href="{{ route('admin.projects.edit', $project) }}" class="menu-link px-3">
                            ...
                            <tr>
                                <td colspan="8" class="text-center py-10">
                                    <div class="d-flex flex-column align-items-center">
                                        ...
                                        <span class="text-gray-600 fs-4 fw-bold mb-2">{{ __('admin.no_projects') }}</span>
                                        <span class="text-muted fs-6">{{ __('admin.no_projects_description') }}</span>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
            ...
            @if(method_exists($projects, 'hasPages') && $projects->hasPages())
                <div class="d-flex justify-content-between align-items-center flex-wrap pt-5">
                    <div class="fs-6 fw-semibold text-gray-700">
                        {{ __('admin.showing') }} {{ $projects->firstItem() }} {{ __('admin.to') }} {{ $projects->lastItem() }}
                        {{ __('admin.of') }} {{ $projects->total() }} {{ __('admin.entries') }}
                    </div>
                    <div>
                        {{ $projects->links() }}
                    </div>
